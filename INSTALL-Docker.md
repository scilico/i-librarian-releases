...
```sh
docker build -t i-librarian:6.1.10 - < i-librarian-6.1.10.tgz
```
...
```sh
tar xzf i-librarian-6.1.10.tgz ./docker-compose.yml
```
...
* Line 19: config directory on the host, default is `/etc/opt/i-librarian`. Extract the configuration file to that location:
    ```sh
    tar xzf i-librarian-6.1.10.tgz ./etc/opt/i-librarian/run-default-docker.env --strip-components=4
